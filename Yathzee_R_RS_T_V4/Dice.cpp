...
Tree* Dice::lrRotate(Tree* ptr) {
    Tree* temp;
    temp = ptr->left;
    ptr->left = rrRotate(temp);
    return llRotate(ptr);
}
Tree* Dice::rlRotate(Tree* ptr) {
    Tree* temp;
    temp = ptr->right;
    ptr->right = llRotate(temp);
    return rrRotate(ptr);
}
Tree* Dice::balance(Tree* temp) {

    int bal_factor = differnce(temp);
    if (bal_factor > 1) {
